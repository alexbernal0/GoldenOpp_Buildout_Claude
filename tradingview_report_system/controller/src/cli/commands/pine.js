...
register('pine', {
  ...
  subcommands: new Map([
    ...
    ['save-as', {
      description: 'Save current Pine Script under a new name — creates a genuine new script slot',
      handler: (opts, positionals) => {
        const name = positionals.join(' ');
        if (!name) throw new Error('Name required. Usage: tv pine save-as "OBQ ADX Signals"');
        return core.saveAs({ name });
      },
    }],
    ['new-tab', {
      description: 'Open a new Pine editor tab (Shift+Alt+T) — genuine new slot, does not overwrite active script',
      handler: () => core.newTab(),
    }],
    ['new', {
