...
export function registerPineTools(server) {
  ...
  server.tool('pine_save_as', 'Save current Pine Script under a new name — creates a genuine new TV cloud script slot without overwriting the active one', {
    name: z.string().describe('New name for the script, e.g. "OBQ ADX Signals"'),
  }, async ({ name }) => {
    try { return jsonResult(await core.saveAs({ name })); }
    catch (err) { return jsonResult({ success: false, error: err.message }, true); }
  });

  server.tool('pine_new_tab', 'Open a new Pine editor tab (Shift+Alt+T) — creates a genuine new script slot so the next pine_set + pine_smart_compile does NOT overwrite the previously added indicator', {}, async () => {
